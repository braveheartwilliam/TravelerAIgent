...
console.log(`Running lightweight Svelte check on: ${targetPath}`);
console.log('======================================');
...
try {
  ...
  let command;
  if (isFile) {
    // For a single file, just lint that file
    command = `npx eslint "${fullPath}" --ext .svelte,.js,.ts`;
  } else {
    // For a directory, lint all Svelte, JS, and TS files in that directory
    command = `npx eslint "${fullPath}" --ext .svelte,.js,.ts`;
  }
  ...
  // We don't use the error object, but we need to catch it
